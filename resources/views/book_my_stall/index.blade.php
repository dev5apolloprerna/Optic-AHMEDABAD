...
@section('title', 'Book My Stall List')
        ...
        <div class="page-content">
            <div class="container-fluid">
                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            ...
                            justify-content: space-between;">
                                <h5 class="card-title mb-0">Book My Stall List</h5>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="container-fluid">
                    ...
                    <div class="card">
                        <div class="card-body">
                            <form method="post" id="form" action="{{ route('brochure.index') }}">
                                @csrf
                                <div class="row  align-items-center">
                                    ...
                                    <div class="col-md-3  mb-2">
                                        <div class="input-group d-flex justify-content-right">
                                            ...
                                            <a class="btn btn-primary" href="{{ route('brochure.index') }}">
                                                Cancel
                                            </a>
                                        </div>
                                    </div>
                            ...
                            <div class="card-body">
                                ...
                                <form role="form" method="POST" action="" name="frmparameter" id="frmparameter"
                                                ...
                                                <tr class="text-center">
                                                    <td data-label="id">
                                                        <input type="checkbox" name="check_list[]"
                                                            ...
                                                            value="<?php echo $Brochures->brochureId; ?>">
                                                        <label for="check_list<?php echo $i; ?>">
                                                            <span></span>
                                                            <span class="check"></span>
                                                            <span class="box"></span></label>
                                                    </td>
                                                    <td>{{ $i + $Brochure->perPage() * ($Brochure->currentPage() - 1) }}
                                                    </td>
                                                    <td>{{ ucwords(strtolower($Brochures->name)) }}</td>
                                                    <td>{{ ucwords(strtolower($Brochures->companyName ?? '-')) }}</td>
                                                    <td>{{ strtolower($Brochures->email) }}</td>
                                                    <td>{{ $Brochures->mobile }}</td>
                                                    <td>{{ $Brochures->city ?? "-" }}</td>
                                                    <td>{{ $Brochures->stall_size ?? "-" }}</td>
                                                    <td>{{ $Brochures->message }}</td>
                                                    <td>{{ date('d-m-Y', strtotime($Brochures->strEntryDate)) }}</td>
                                                    <td>
                                                        <a class="mx-1" href="#" data-bs-toggle="modal"
                                                            title="Delete" data-bs-target="#deleteRecordModal"
                                                            onclick="deleteData(<?= $Brochures->brochureId ?>);">
                                    ...
                                    <div class="d-flex justify-content-center mt-3">
                                        {{ $Brochure->appends(request()->except('page'))->links() }}
                                    </div>
                                </form>
                            </div>
                        </div>
                ...
                <div class="modal-body">
                    ...
                    <div class="d-flex gap-2 justify-content-center mt-4 mb-2">
                        ...
                        <form id="user-delete-form" method="POST"
                            action="{{ route('brochure.delete', $Brochures->brochureId ?? '') }}">
                            @csrf
                            @method('DELETE')
                            <input type="hidden" name="brochureId" id="deleteid" value="">
    ...
    <script>
        function multiDelete() {
            ...
            if (confirm('Are You Sure You want to Delete?')) {
                $.ajax({
                    type: 'DELETE',
                    url: "{{ route('brochure.deleteselected') }}",
                    ...
                    success: function(response) {
                        ...
                        if (response == 1) {
                            ...
                            alert('Book My Stall Deleted Sucessfully.');
